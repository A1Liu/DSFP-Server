...
public class UndirectedGraph extends Graph<Vertex<Integer>> {
	...
	@Override
	public ArrayList<EdgePair> listEdges() throws InterruptedException {//I need to use a breadth-first algorithm here to improve computation time.

		if (size() == 0)
			return null;
		
		//creating necessary containers
		Hashtable<Integer, Boolean> reached = new Hashtable<Integer,Boolean>();//hashtable makes checking which nodes have been reached into a linear operation
		Queue<Integer> queue = new Queue<Integer>();//The nodes to check
		ArrayList<EdgePair> edgeList = new ArrayList<EdgePair>();//List of edges to be outputted
		Enumeration<Integer> vertexList = enumKeys();//list of all vertices to make sure completely isolated subgraphs aren't ignored
		Integer current;
		EdgeList currentEdges;//Current edges
		Integer currentNeighbor;//current neighbor of current vertex
		Integer start = getVertex(vertexList.nextElement()).getLabel();//The vertex the algorithm starts on
		
		//keep going until the entire network is traversed, including all isolated graphs
		while (reached.size()<size()) {
			...
			if (!reached.containsKey(start)) {
				...
				while (!queue.isEmpty()) {
					...
					if (!reached.containsKey(current)) {//make sure we don't iterate over the same things over and over
						...
						for (int x = 0; x<currentEdges.size();x++) {
							...
							if (!reached.containsKey(currentNeighbor)) {
								edgeList.add(new EdgePair(getVertex(current),currentEdges.get(x)));//if the neighbor hasn't been reached, add the edge
								queue.enqueue(currentNeighbor);//only add the neighbor to the queue if it wasn't reached
							}	
						}
			...
			if (vertexList.hasMoreElements())//make sure that separated graphs aren't missed based on the starting node.
